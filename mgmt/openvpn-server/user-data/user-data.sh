...
function init_openvpn {
  local readonly ca_country="$1"
  local readonly ca_state="$2"
  local readonly ca_locality="$3"
  local readonly ca_org="$4"
  local readonly ca_org_unit="$5"
  local readonly ca_email="$6"
  local readonly backup_bucket_name="$7"
  local readonly kms_key_id="$8"
  local readonly key_size="$9"
  shift 9
  local readonly ca_expiration_days="$1"
  local readonly cert_expiration_days="$2"
  local readonly vpn_subnet="$3"
  shift 3
  local routes=()

  while [[ $# -gt 0 ]]; do
    local readonly route="$1"
